...
**Why this works better than a generic navy/amber system:** the cyan is already doing the job of "premium but approachable" in the existing logo — reusing it as the single primary action color means every button, link and form field on the site visually reinforces the brand mark instead of introducing an unrelated color. Blue (navy + sky, together) should read as the site's dominant color story — it's what's in the logo, it's most of the hero photo's sky and reflections, and it should own headers, footers, section backgrounds, icon fills, dividers and every interactive element. Gold is demoted further, to a *rare accent*: a small icon fill, a thin divider rule, a badge on a pilgrimage/wedding/heritage page — never a background, never a large shape, never more than one gold element visible in a given viewport. If a section doesn't have a specific heritage/pilgrimage/wedding story to tell, it should have no gold in it at all.

**Rule of thumb for build/design QA:** in any given screen, blue (navy + sky combined) should account for roughly 85–95% of all color usage; gold should never exceed a few percent, and should be visibly the exception, not a recurring pattern. If a page "feels" like it has two co-equal accent colors, gold is being overused.
...
1. **Blue is the dominant color family sitewide** — `navy-900`/`navy-700` for authority (logo, header, footer, primary headlines, dark section backgrounds) and `sky-500` for everything actionable (CTAs, links, active nav, form focus, icon fills, hover states). Between the two, blue should visually own every page.
...
3. **Gold (`heritage-gold`)** is a rare, deliberate accent, not a second brand color — a single small icon fill, a thin rule, or a badge on a pilgrimage/wedding/heritage page. It should never appear as a background fill, a large shape, or alongside sky-blue CTAs in a way that makes them compete. Most pages (corporate mobility, employee transport, airport transfers) should have zero gold.
